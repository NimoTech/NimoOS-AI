...
def add_memory(conn, user_id, text, kind="fact", *, source="auto",
               trust="normal", priority=0, origin_session_id=None,
               lineage_id=None, expires_at=None, now=None) -> str:
    ...
    conn.execute(
        ...
        "(id, user_id, kind, text, source, trust, priority, status, lineage_id, "
        " supersedes, recall_count, last_recalled_at, created_at, updated_at, "
        " expires_at, origin_session_id) "
        "VALUES (?,?,?,?,?,?,?,'active',?,NULL,0,?,?,?,?,?)",
        (mem_id, user_id, kind, text, source, trust, priority, lineage,
...
def render_user_block(conn, user_id, *, now=None,
    ...
    now = now if now is not None else int(time.time())
    # Low-trust memory (extracted from channel-sourced sessions that may
    # contain untrusted/injected content) is excluded from system-prompt
    # injection, but remains stored and visible via list_active/the
    # memory-management UI.
    rows = conn.execute(
        "SELECT * FROM memory_entries "
        "WHERE user_id=? AND status='active' AND trust != 'low' "
        "AND (expires_at IS NULL OR expires_at > ?)",
        (user_id, now),
    ).fetchall()
    if not rows:
